...
#if TARGET_OS_IPHONE
#ifdef __LP64__
    //iOS 64位系统 在iOS7.1之前。需要特别替换methodSignatureForSelector方法
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7.1) {
...
static NSDictionary *defineClass(NSString *classDeclaration, JSValue *instanceMethods, JSValue *classMethods)
{
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        for (NSString *jsMethodName in methodDict.allKeys) {
            ...
            //如果原来的类已经有这样的方法，则进行替换
            if (class_respondsToSelector(currCls, NSSelectorFromString(selectorName))) {
                overrideMethod(currCls, selectorName, jsMethod, !isInstance, NULL);
            } else {
                //要动态添加这个方法
                BOOL overrided = NO;
                //如果有protocol的声明，则直接通过protocol拿到方法typeEncoding
                for (NSString *protocolName in protocols) {
                    //
...
static void JPForwardInvocation(__unsafe_unretained id assignSlf, SEL selector, NSInvocation *invocation)
{
    ...
    //通过全局静态字典拿到当前selector绑定的JSValue。
    ...
    if ([slf class] == slf) {
        ...
        //如果Hook的是dealloc方法。 构造JSPboxing是需要 assign描述。 因为dealloc使用weak会触发二次释放，引发运行时崩溃
    ...
    for (NSUInteger i = 2; i < numberOfArguments; i++) {
        ...
        switch(argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
            ...
            case '@': {
                //如果参数是OC对象，使用__unsafe_unretained,避免触发objc_retain
                __unsafe_unretained id arg;
                [invocation getArgument:&arg atIndex:i];
                if ([arg isKindOfClass:NSClassFromString(@"NSBlock")]) {
                    //防止Block在调用时崩溃。 存放到容器时需要copy
    ...
    //如果是dealloc，则还需要在执行完之后恢复dealloc的调用
    if (deallocFlag) {
        slf = nil;
...
static void overrideMethod(Class cls, NSString *selectorName, JSValue *function, BOOL isClassMethod, const char *typeDescription)
{
    ...
    //注意在32位系统中寄存器无法保存返回值为struct的情况，因此采用了_objc_msgForward_stret函数.
    #if !defined(__arm64__)
        if (typeDescription[0] == '{') {
            //In some cases that returns struct, we should use the '_stret' API:
            //http://sealiesoftware.com/blog/archive/2008/10/30/objc_explain_objc_msgSend_stret.html
            //NSMethodSignature knows the detail but has no API to return, we can only get the info from debugDescription.
            //这里用了一个非常取巧的方式来判断当前的方法是否返回值是Struct。如果是struct，方法签名中会有这么一行描述。。。
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    //Hook forwardInvocation。注意这里可能会跟Aspect冲突。
...
#pragma clang diagnostic pop
    ...
    //替换掉原来的selector，当调用时执行的是替换的msgForward.
